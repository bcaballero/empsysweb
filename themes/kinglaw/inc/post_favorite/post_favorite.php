...
class NectarLove {
	...
	function love_post($post_id, $action = 'get')
	{
		...
		switch($action) {
			...
			case 'get':
				...
				return '<span class="nectar-love-count"><i class="zmdi zmdi-favorite-outline"></i>'. $love_count .'</span>';
				break;
			...
			case 'update':
				...
				return '<span class="nectar-love-count"><i class="zmdi zmdi-favorite-outline"></i>'. $love_count .' <span class="text"> '. esc_html__('LIKE','kinglaw') .' </span></span>';
				break;
	...
	function add_love($unit = '',$show_icon = true) {
		...
		$output = $this->love_post($post->ID);
  		...
  		$class = 'nectar-love';
  		$icon = 'material-icons';
  		$title = esc_html__('Love this', 'kinglaw');
		if( isset($_COOKIE['nectar_love_'. $post->ID]) ){
			$class = 'nectar-love loved';
			$icon = 'material-icons';
			$title = esc_html__('You already love this!', 'kinglaw');
		}
