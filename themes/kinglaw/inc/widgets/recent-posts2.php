<?php
add_action('widgets_init', 'cs_recent_post2_carousel');

function cs_recent_post2_carousel() {
    register_widget('CS_Recent_Post2_Carousel');
}

class CS_Recent_Post2_Carousel extends WP_Widget {

    function __construct() {
        parent::__construct(
                'cs_recent_post2_carousel', esc_html__('CMS Recent Post style2', 'kinglaw'), array('description' => esc_html__('Recent Posts Widget.', 'kinglaw'),)
        );
    }
    ...
    function widget($args, $instance) {
        ...
        $title = apply_filters('widget_title', empty($instance['title']) ? esc_html__('Recent Posts', 'kinglaw' ) : $instance['title'], $instance, $this->id_base);
        $show_date = (int) $instance['show_date'];
        $show_decs = (int) $instance['show_decs'];
        $number = (int) $instance['number'];

        echo '<aside id="cms-recent-post-style">';
            if($title) {
    ...
    function form($instance) {
        ...
        ?>
        <p>
            <label for="<?php echo ''.$this->get_field_id('title'); ?>"><?php esc_html_e( 'Title:', 'kinglaw' ); ?></label>
        ...
        <p>
            <label for="<?php echo ''.$this->get_field_id('number'); ?>"><?php esc_html_e( 'Number of post to show:', 'kinglaw' ); ?></label>
